...
// const planetNames = ["Tatooine", "Alderaan", "Yavin IV", "Hoth", "Dagobah", "Bespin", "Endor", "Naboo", "Coruscant", "Kamino"]

beforeEach(() => {
  jest.spyOn(global, 'fetch');
  global.fetch.mockResolvedValue({
    json: jest.fn().mockResolvedValue(mockFetchApi)
  });
});
...
test('Verifica se o select para filtro de coluna possui os options corretos', () => {
  ...
  options.forEach(option => {
    expect(selectColumn).toHaveTextContent(option);
  });
});
...
test('Verifica se a API é chamada', () => {
  ...
  test('Insere um filtro e verifica se o retorno é correto', async () => {
      render(<App />);
